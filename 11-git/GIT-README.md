...
Выполнена в отдельной папке, т.к. в Readme репозитория будет размещено описание всего учебного проекта. 
...
### Stash
Операция берёт изменённое состояние вашего рабочего каталога и сохраняет его в хранилище незавершённых изменений.  
Изменения можно в любое время применить обратно.
...
#### Отложить отдельные изменения
```sh
git stash -p 
```
Команда будет выполняться для каждого измененного участка кода, запрашивая подтверждение на откладывание

#### Просмотреть список созданных наборов
```sh
git stash list # просмотреть список
...
### Worktree
Для меня это открытие недавнего времени. Позволяет работать с каждой веткой одного репозитория в разных папках. Подробности [тут](https://t.me/memodyadka/67)
```sh
